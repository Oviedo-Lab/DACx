...
The following functions have implementation changes that are not
reflected in their documentation comments.
...
- The function uses `return_arbors` to optionally drop the `arbors`
  field from the returned list (see lines 704-706)
- This is documented in the code comments as being useful for keeping
  the returned object cacheable by knitr
...
Default value for `soma_size_factor` parameter differs between
...
Default value for `return_plot` parameter differs between documentation
and implementation.
...
✓ **This is CORRECT** — no update needed. The documentation and
implementation match (both TRUE).

However, the documentation at line 774 shows `return_plot = FALSE`,
...
Default value for `return_cell_arbor_idx` parameter differs between
documentation and implementation.
...
However, the documentation at line 775 shows
`return_cell_arbor_idx = FALSE`, which is inconsistent.
...
|----|----|----|----|
| [`fetch.network.components()`](https://Oviedo-Lab.org/DACx/reference/fetch.network.components.md) | 670-677 | `return_arbors` | Add missing parameter documentation |
| [`plot.network()`](https://Oviedo-Lab.org/DACx/reference/plot-network.md) | 773 | `soma_size_factor` | Change default from 3.0 to 0.5 in usage section |
| [`plot.network()`](https://Oviedo-Lab.org/DACx/reference/plot-network.md) | 774 | `return_plot` | Change default from FALSE to TRUE in usage section |
| [`plot.network()`](https://Oviedo-Lab.org/DACx/reference/plot-network.md) | 775 | `return_cell_arbor_idx` | Change default from FALSE to TRUE in usage section |
| [`plot.network()`](https://Oviedo-Lab.org/DACx/reference/plot-network.md) | 789 | `soma_size_factor` | Add/clarify default value (0.5) in parameter description |
